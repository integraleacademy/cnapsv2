...
def add():
    ...
    with sqlite3.connect(DB) as conn:
        conn.execute("INSERT INTO dossiers (nom, prenom, formation, session) VALUES (?, ?, ?, ?)", (nom, prenom, formation, session))
    return redirect("/")
...
def attestation(id):
    ...
    template_path = "fichier_aps.docx" if formation == "APS" else "fichier_a3p.docx"
    doc = Document(template_path)
    for p in doc.paragraphs:
        p.text = p.text.replace("{{nom}}", nom).replace("{{prenom}}", prenom).replace("{{session}}", session)
    doc.add_picture("static/tampon_signature.png", width=docx.shared.Inches(4))
    file_stream = BytesIO()
    doc.save(file_stream)
    file_stream.seek(0)
    return send_file(file_stream, as_attachment=True, download_name=f"attestation_{nom}_{prenom}.docx")
